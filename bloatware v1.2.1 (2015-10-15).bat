...
if /i %SCRIPT_VERSION% LSS %REPO_SCRIPT_VERSION% (
	...
	if !CHOICE!==y (
		...
		echo %TIME%   Press any key to start the newest version
		pause>NUL
		call "bloatware v%REPO_SCRIPT_VERSION% (%REPO_SCRIPT_DATE%).bat"
		exit
		)
	)
	color 0f
...
color CF
echo.
echo Would you like to remove Office 2013 and 2016 Click-to-run (OEM) that normally comes with new PCs, this can take up on average around 1-2GB. However if you wish to install Office 2013 (OEM) on their PC at a later date you will have to redownload it from scratch again and depending on their internet speed this may take a while.
echo.              
set /P RemoveOffice=Uninstall Office 2013 and 2016 C2R [Y]/n?
echo.
if /I "%RemoveOffice%"==Y (
...
if exist "C:\Program Files (x86)\Lenovo\Customer Feedback Program\Lenovo.TVT.CustomerFeedback.Agent.exe" del "C:\Program Files (x86)\Lenovo\Customer Feedback Program\Lenovo.TVT.CustomerFeedback.Agent.exe" 2>NUL
if exist "C:\Program Files (x86)\Lenovo\Customer Feedback Program\Lenovo.TVT.CustomerFeedback.Agent.exe.config" del "C:\Program Files (x86)\Lenovo\Customer Feedback Program\Lenovo.TVT.CustomerFeedback.Agent.exe.config" 2>NUL
if exist "C:\Program Files (x86)\Lenovo\Customer Feedback Program\Lenovo.TVT.CustomerFeedback.InnovApps.dll" del "C:\Program Files (x86)\Lenovo\Customer Feedback Program\Lenovo.TVT.CustomerFeedback.InnovApps.dll" 2>NUL
if exist "C:\Program Files (x86)\Lenovo\Customer Feedback Program\Lenovo.TVT.CustomerFeedback.OmnitureSiteCatalyst.dll" del "C:\Program Files (x86)\Lenovo\Customer Feedback Program\Lenovo.TVT.CustomerFeedback.OmnitureSiteCatalyst.dll" 2<NUL
...
cls 
if /i %TempCleanup%==Y 	( 
		echo Running TempFileCleanup now
		start /wait TempFileCleanup.bat 
